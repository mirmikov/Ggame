  ...
  return (
    <section className="hero grid-bg">
      ...
      <div className="panel access-panel">
        <label>
          НАЗВАНИЕ КОМАНДЫ / ПОЗЫВНОЙ
          <input
            value={session.nickname}
            maxLength={24}
            placeholder="Например, Алгоритмисты"
            onChange={(e) =>
        ...
        {form.gameMode === "qualifier" && (
          <>
            ...
            <div className="qualifier-rule-note">
              <b>Важно:</b> запуск доступен только при чётном числе подключённых
              команд. В финал проходит ровно половина.
            </div>
          </>
        )}
        <button
          ...
          onClick={() =>
            create({
              ...
              settings: {
                ...form.settings,
                teamPlayerLimit: Math.ceil(form.maxPlayers / 2),
              },
            })
          }
        >
      ...
      >
        <div className="organizer-lobby">
          <div className="panel lobby-code">
            ...
            <p>
              Покажите код командам. Для отбора одна команда подключается с
              одного устройства.
            </p>
          </div>
          <div className="panel">
            <h2>{room.gameMode === "qualifier" ? "ЗАХВАТ ЗОНЫ" : "ФИНАЛ"}</h2>
            <div className="setting-list">
              ...
              {room.gameMode === "qualifier" && (
                <>
                  <span>
                    В ФИНАЛ ПРОЙДЁТ <b>{Math.floor(participants.length / 2)}</b>
                  </span>
                  <span>
                    ШАГОВ ДО ЗОНЫ <b>{room.settings.zoneStepsToCenter}</b>
                  </span>
                  <span>
                    УДЕРЖАНИЕ <b>{room.settings.zoneHoldSeconds} сек</b>
                  </span>
                </>
              )}
            </div>
            {room.gameMode === "qualifier" && !qualifierReady && (
              <p className="lobby-warning">
                Нужно минимум 2 команды и обязательно чётное количество.
              </p>
            )}
            <button
          ...
          <div className="panel participant-board">
            <h3>ПОДКЛЮЧИВШИЕСЯ КОМАНДЫ</h3>
            {participants.length ? (
              participants.map((p) => (
                <ParticipantRow key={p.id} player={p} mode={room.gameMode} />
              ))
            ) : (
              <p className="waiting">Ожидание команд...</p>
            )}
          </div>
        </div>
      </Screen>
  ...
  return (
    ...
    >
      <div className="participant-lobby panel">
        ...
        <h2>ВЫ ПОДКЛЮЧЕНЫ</h2>
        <p>Организатор запустит тур после подключения всех команд.</p>
        {room.gameMode === "qualifier" ? (
          <div className="qualifier-team">
            <b>ЗАХВАТ ЦЕНТРАЛЬНОЙ ЗОНЫ</b>
            <span>
              Правильный ответ двигает вашу команду к центру. Если вы ворвётесь
              в занятую зону, её владелец будет отброшен назад.
            </span>
            <span>В финал проходит ровно половина команд.</span>
          </div>
        ) : (
...
function ParticipantRow({
  ...
  return (
    <div className="participant-row">
      ...
      <em>{mode === "qualifier" ? "команда" : player.team || "ожидает"}</em>
    </div>
  );
}
...
function qualifierStatusLabel(player: Player) {
  if (player.qualifierStatus === "holding") return "В ЗОНЕ";
  if (player.qualifierStatus === "qualified") return "ПРОШЛА В ФИНАЛ";
  if (player.qualifierStatus === "eliminated") return "ВЫБЫЛА";
  return "ДВИЖЕТСЯ";
}
...
function QualifierArena({ room }: { room: Room }) {
  ...
  const ordered = [...participants].sort(
    (a, b) => (room.units[a.id]?.lane ?? 0) - (room.units[b.id]?.lane ?? 0),
  );
  const holder = room.zoneHolderId
    ? room.players[room.zoneHolderId]
    : undefined;
  ...
  return (
    <section className="game-screen organizer-arena">
      <div className="projector-banner">
        <b>ОТБОРОЧНЫЙ ТУР // ЗАХВАТ ЦЕНТРАЛЬНОЙ ЗОНЫ</b>
      ...
      <div className="zone-top">
        <div className="zone-summary">
          ...
          <b>
            {qualified.size} /{" "}
            {room.qualifierSlots || Math.floor(participants.length / 2)}
          </b>
        </div>
        <div className="timer">
        ...
        <div className={`zone-holder-card ${holder ? "occupied" : ""}`}>
          ...
          <b>{holder?.nickname || "СВОБОДНА"}</b>
          <i>
            <em style={{ width: `${holdPct}%` }} />
          </i>
          <span>
            {holder
              ? `${holder.captureProgress || 0}/${room.settings.zoneHoldSeconds} сек`
      ...
      <div className="arena zone-arena">
        ...
        >
          {ordered.map((p) => {
            const unit = room.units[p.id];
            const point = radialPoint(unit?.lane || 0, 0, ordered.length);
            return (
              <line key={p.id} x1={point.x} y1={point.y} x2="50" y2="50" />
            );
          })}
        </svg>
        ...
        {ordered
          ...
          .map((p, index) => {
            ...
            return (
              <div
                key={p.id}
                className={`zone-team ${p.qualifierStatus || "active"}`}
                style={
                  {
                    left: `${point.x}%`,
                    top: `${point.y}%`,
                    "--team-hue": `${(index * 53) % 360}`,
                  } as CSSProperties
                }
              >
                ...
                <b>{p.nickname}</b>
                <small>
                  {p.zoneSteps || 0}/{room.settings.zoneStepsToCenter} шагов
                </small>
              </div>
            );
          })}
      ...
      <div className="organizer-bottom">
        ...
        <div className="panel live-roster">
          ...
          {[...participants]
            .sort(
              (a, b) =>
                (b.zoneSteps || 0) - (a.zoneSteps || 0) || b.score - a.score,
            )
            .map((p) => (
              <div key={p.id}>
                <b>{p.nickname}</b>
                <span>
                  {qualifierStatusLabel(p)} // {p.captureProgress || 0} сек
                </span>
                <strong>
                  {p.zoneSteps || 0}/{room.settings.zoneStepsToCenter}
                </strong>
              </div>
            ))}
        </div>
...
function ParticipantConsole({
  ...
  const qualifierDone =
    room.gameMode === "qualifier" &&
    (player?.qualifierStatus === "qualified" ||
      player?.qualifierStatus === "eliminated");
  const holder = room.zoneHolderId === player?.id;
  const stepsRemaining = Math.max(
    0,
    room.settings.zoneStepsToCenter - (player?.zoneSteps || 0),
  );
  const statusText =
    player?.qualifierStatus === "qualified"
      ? "ВЫ ПРОШЛИ В ФИНАЛ"
  ...
  return (
    <section className="participant-console">
      ...
      <div className={`buff-strip ${holder ? "zone-owned" : ""}`}>
        ...
        <b>
          {room.gameMode === "qualifier"
            ? `${statusText} // ${player?.latestBuff || "Решайте задания"}`
      ...
      {room.gameMode === "qualifier" ? (
        <div className="participant-stats">
          <Stat
            name="ШАГИ"
            value={`${player?.zoneSteps || 0}/${room.settings.zoneStepsToCenter}`}
          />
          <Stat
            name="ЗАХВАТ"
            value={`${player?.captureProgress || 0}/${room.settings.zoneHoldSeconds}`}
          />
      ...
      <div className="panel challenge-panel">
        {qualifierDone ? (
          <div className={`qualifier-finish ${player?.qualifierStatus}`}>
            <b>
              {player?.qualifierStatus === "qualified"
                ? "ФИНАЛ ДОСТУПЕН"
                : "СПАСИБО ЗА ИГРУ"}
            ...
            <span>
              {player?.qualifierStatus === "qualified"
                ? "Ваша команда вошла в число сильнейших. Ожидайте запуск финального лобби."
                : "В следующий тур прошла половина команд. Результаты видны на экране организатора."}
            </span>
          </div>
        ) : (
          <>
      ...
      Object.values(room.players)
        ...
        .sort((a, b) => {
          if (room.gameMode === "qualifier") {
            const aq = a.qualifierStatus === "qualified" ? 1 : 0;
            const bq = b.qualifierStatus === "qualified" ? 1 : 0;
            return (
              bq - aq ||
              (b.zoneSteps || 0) - (a.zoneSteps || 0) ||
              b.score - a.score
            );
          }
    ...
    return (
      <Screen title="ОТБОРОЧНЫЙ ТУР // РЕЗУЛЬТАТЫ">
        <div className="winner blue-text">
          ...
          {room.qualifierSlots || Math.floor(players.length / 2)} КОМАНД
        </div>
        <p className="result-story">{room.storyMessage}</p>
        <div className="panel leaderboard qualifier-results">
          {players.map((p, i) => (
            <div key={p.id} className={p.qualifierStatus || "eliminated"}>
              <b>#{i + 1}</b>
              <span>
                {p.nickname}
                <small>{p.grade} класс</small>
              </span>
              <span>
                {p.qualifierStatus === "qualified" ? "ФИНАЛ" : "ВЫБЫЛА"}
              </span>
              <span>
                {p.zoneSteps || 0}/{room.settings.zoneStepsToCenter} шагов
              </span>
